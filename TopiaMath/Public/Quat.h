...
namespace topia
{
	...
	public:
		...
		/// If this quaternion is close to inRHS. Note that q and -q represent the same rotation, this is not checked
		/// here.
		inline bool IsClose(QuatArg inRHS, float inMaxDistSq = 1.0e-12f) const
		{
			return mValue.IsClose(inRHS.mValue, inMaxDistSq);
		}
		...
		TOPIA_INLINE float GetX() const { return mValue.GetX(); }

		/// Get Y component (imaginary part j)
		TOPIA_INLINE float GetY() const { return mValue.GetY(); }

		/// Get Z component (imaginary part k)
		TOPIA_INLINE float GetZ() const { return mValue.GetZ(); }

		/// Get W component (real part)
		TOPIA_INLINE float GetW() const { return mValue.GetW(); }

		/// Get the imaginary part of the quaternion
		TOPIA_INLINE Vec3 GetXYZ() const { return Vec3(mValue); }

		/// Get the quaternion as a Vec4
		TOPIA_INLINE Vec4Arg GetXYZW() const { return mValue; }
		...
		/// @return [0, 0, 0, 0]
		TOPIA_INLINE static Quat sZero() { return Quat(Vec4::sZero()); }

		/// @return [1, 0, 0, 0] (or in storage format Quat(0, 0, 0, 1))
		TOPIA_INLINE static Quat sIdentity() { return Quat(0, 0, 0, 1); }

		///@}

		/// Rotation from axis and angle
		TOPIA_INLINE static Quat sRotation(Vec3Arg inAxis, float inAngle);

		/// Get axis and angle that represents this quaternion, outAngle will always be in the range \f$[0, \pi]\f$
		TOPIA_INLINE void GetAxisAngle(Vec3 &outAxis, float &outAngle) const;

		/// Create quaternion that rotates a vector from the direction of inFrom to the direction of inTo along the
		/// shortest path
		/// @see https://www.euclideanspace.com/maths/algebra/vectors/angleBetween/index.htm
		TOPIA_INLINE static Quat sFromTo(Vec3Arg inFrom, Vec3Arg inTo);
		...
		TOPIA_INLINE float LengthSq() const { return mValue.LengthSq(); }

		/// Length of quaternion.
		/// @return Length of quaternion (\f$|v|\f$)
		TOPIA_INLINE float Length() const { return mValue.Length(); }

		/// Normalize the quaternion (make it length 1)
		TOPIA_INLINE Quat Normalized() const { return Quat(mValue.Normalized()); }

		///@}
		///@name Additions / multiplications
		///@{

		TOPIA_INLINE void operator+=(QuatArg inRHS) { mValue += inRHS.mValue; }
		TOPIA_INLINE void operator-=(QuatArg inRHS) { mValue -= inRHS.mValue; }
		TOPIA_INLINE void operator*=(float inValue) { mValue *= inValue; }
		TOPIA_INLINE void operator/=(float inValue) { mValue /= inValue; }
		TOPIA_INLINE Quat operator-() const { return Quat(-mValue); }
		TOPIA_INLINE Quat operator+(QuatArg inRHS) const { return Quat(mValue + inRHS.mValue); }
		TOPIA_INLINE Quat operator-(QuatArg inRHS) const { return Quat(mValue - inRHS.mValue); }
		TOPIA_INLINE Quat operator*(QuatArg inRHS) const;
		TOPIA_INLINE Quat operator*(float inValue) const { return Quat(mValue * inValue); }
		...
		TOPIA_INLINE Vec3 operator*(Vec3Arg inValue) const;

		/// Rotate a vector by the inverse of this quaternion
		TOPIA_INLINE Vec3 InverseRotate(Vec3Arg inValue) const;

		/// Rotate a the vector (1, 0, 0) with this quaternion
		TOPIA_INLINE Vec3 RotateAxisX() const;

		/// Rotate a the vector (0, 1, 0) with this quaternion
		TOPIA_INLINE Vec3 RotateAxisY() const;

		/// Rotate a the vector (0, 0, 1) with this quaternion
		TOPIA_INLINE Vec3 RotateAxisZ() const;

		/// Dot product
		TOPIA_INLINE float Dot(QuatArg inRHS) const { return mValue.Dot(inRHS.mValue); }

		/// The conjugate [w, -x, -y, -z] is the same as the inverse for unit quaternions
		TOPIA_INLINE Quat Conjugated() const
		{
			return Quat(Vec4::sXor(mValue, UVec4(0x80000000, 0x80000000, 0x80000000, 0).ReinterpretAsFloat()));
		}

		/// Get inverse quaternion
		TOPIA_INLINE Quat Inversed() const { return Conjugated() / Length(); }

		/// Ensures that the W component is positive by negating the entire quaternion if it is not. This is useful when
		/// you want to store a quaternion as a 3 vector by discarding W and reconstructing it as sqrt(1 - x^2 - y^2 -
		/// z^2).
		TOPIA_INLINE Quat EnsureWPositive() const
		{
			return Quat(
			    Vec4::sXor(mValue, Vec4::sAnd(mValue.SplatW(), UVec4::sReplicate(0x80000000).ReinterpretAsFloat())));
		}

		/// Get a quaternion that is perpendicular to this quaternion
		TOPIA_INLINE Quat GetPerpendicular() const
		{
			return Quat(Vec4(1, -1, 1, -1) * mValue.Swizzle<SWIZZLE_Y, SWIZZLE_X, SWIZZLE_W, SWIZZLE_Z>());
		}

		/// Get rotation angle around inAxis (uses Swing Twist Decomposition to get the twist quaternion and uses
		/// q(axis, angle) = [cos(angle / 2), axis * sin(angle / 2)])
		TOPIA_INLINE float GetRotationAngle(Vec3Arg inAxis) const
		{
			return GetW() == 0.0f ? TOPIA_PI : 2.0f * atan(GetXYZ().Dot(inAxis) / GetW());
		}
		...
		TOPIA_INLINE Quat GetTwist(Vec3Arg inAxis) const;
		...
		TOPIA_INLINE void GetSwingTwist(Quat &outSwing, Quat &outTwist) const;
		...
		TOPIA_INLINE Quat LERP(QuatArg inDestination, float inFraction) const;
		...
		TOPIA_INLINE Quat SLERP(QuatArg inDestination, float inFraction) const;

		/// Load 3 floats from memory (X, Y and Z component and then calculates W) reads 32 bits extra which it doesn't
		/// use
		static TOPIA_INLINE Quat sLoadFloat3Unsafe(const Float3 &inV);

		/// Store 3 as floats to memory (X, Y and Z component)
		TOPIA_INLINE void StoreFloat3(Float3 *outV) const;

		/// To String
		friend std::ostream &operator<<(std::ostream &inStream, QuatArg inQ)
		{
			inStream << inQ.mValue;
			return inStream;
		}
