...
TOPIA_MAKE_HASHABLE(topia::Vec3, t.GetX(), t.GetY(), t.GetZ())
...
namespace topia
{
	...
	{
#ifdef TOPIA_FLOATING_POINT_EXCEPTIONS_ENABLED
		// Avoid asserts when both components are NaN
		ASSERT(reinterpret_cast<const u32 *>(mF32)[2] == reinterpret_cast<const u32 *>(mF32)[3]);
#endif // TOPIA_FLOATING_POINT_EXCEPTIONS_ENABLED
	}

	TOPIA_INLINE Vec3::Type Vec3::sFixW(Type inValue)
	{
#ifdef TOPIA_FLOATING_POINT_EXCEPTIONS_ENABLED
...
#error Unsupported CPU architecture
	...
	Vec3::Vec3(Vec4Arg inRHS) : mValue(sFixW(inRHS.mValue))
	{
	}
...
#elif defined(TOPIA_USE_NEON)
		uint32x2_t xy = vcreate_f32(static_cast<uint64>(*reinterpret_cast<u32 *>(&inX)) | (static_cast<uint64>(*reinterpret_cast<u32 *>(&inY)) << 32));
		uint32x2_t zz = vcreate_f32(static_cast<uint64>(*reinterpret_cast<u32 *>(&inZ)) | (static_cast<uint64>(*reinterpret_cast<u32 *>(&inZ)) << 32));
		mValue = vcombine_f32(xy, zz);
#else
#error Undefined CPU architecture
#endif
...
#error Unsupported CPU architecture
	...
	Vec3 Vec3::sNaN()
	{
		return sReplicate(std::numeric_limits<float>::quiet_NaN());
	}
...
#error Unsupported CPU architecture
	...
	Vec3 Vec3::sRandom(Random &inRandom)
	{
		...
		float theta = TOPIA_PI * zero_to_one(inRandom);
		float phi = 2.0f * TOPIA_PI * zero_to_one(inRandom);
