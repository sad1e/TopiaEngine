...
{
// Constructor
Vec4::Vec4(Vec3Arg inRHS) : mValue(inRHS.mValue)
{
}
...
Vec4::Vec4(float inX, float inY, float inZ, float inW)
{
    ...
    uint32x2_t zw = vcreate_f32(static_cast<uint64>(*reinterpret_cast<u32*>(&inZ)) | (static_cast<uint64>(*reinterpret_cast<u32*>(&inW)) << 32));
...
{
    #if defined(TOPIA_USE_SSE)
    #ifdef TOPIA_USE_AVX2
    return _mm_i32gather_ps(inBase, inOffsets.mValue, Scale);
    #else
    const u8* base = reinterpret_cast<const u8*>(inBase);
