...
bool EigenValueSymmetric(const Matrix &inMatrix, Matrix &outEigVec, Vector &outEigVal)
{
	...
	ASSERT(inMatrix.GetRows() == n);
	ASSERT(inMatrix.GetCols() == n);
	ASSERT(outEigVec.GetRows() == n);
	ASSERT(outEigVec.GetCols() == n);
	ASSERT(outEigVal.GetRows() == n);
	ASSERT(outEigVec.IsIdentity());
	...
	for (int sweep = 0; sweep < cMaxSweeps; ++sweep)
	{
		...
		{
			// Sanity checks
			#ifdef TOPIA_ENABLE_ASSERTS
				for (uint c = 0; c < n; ++c)
				{
					// Check if the eigenvector is normalized
					ASSERT(outEigVec.GetColumn(c).IsNormalized());
					...
					ASSERT(mat_eigvec.IsClose(eigval_eigvec, max(mat_eigvec.LengthSq(), eigval_eigvec.LengthSq()) * 1.0e-6f));
				}
			#endif
			...
			for (uint iq = ip + 1; iq < n; ++iq)
			{
				...
				else if (abs(a(ip, iq)) > tresh)
				{
					...
					if (!isnan(h))
					{
						...
						#define TOPIA_EVS_ROTATE(a, i, j, k, l)		\
							g = a(i, j),							\
							h = a(k, l),							\
							a(i, j) = g - s * (h + g * tau),		\
							a(k, l) = h + s * (g - h * tau)
						...
						uint j;
						for (j = 0; j < ip; ++j)		TOPIA_EVS_ROTATE(a, j, ip, j, iq);
						for (j = ip + 1; j < iq; ++j)	TOPIA_EVS_ROTATE(a, ip, j, j, iq);
						for (j = iq + 1; j < n; ++j)	TOPIA_EVS_ROTATE(a, ip, j, iq, j);
						for (j = 0; j < n; ++j)			TOPIA_EVS_ROTATE(outEigVec, j, ip, j, iq);

						#undef TOPIA_EVS_ROTATE
					}
				}
			}
