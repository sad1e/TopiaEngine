namespace topia
{
	...
	Quat Quat::sRandom(Random& inRandom)
	{
		...
		std::uniform_real_distribution<float> zero_to_two_pi(0.0f, 2.0f * TOPIA_PI);
	...
	Quat Quat::sEulerAngles(Vec3Arg inAngles)
	{
		...
		return Quat(cz * sx * cy - sz * cx * sy, cz * cx * sy + sz * sx * cy, sz * cx * cy - cz * sx * sy,
		            cz * cx * cy + sz * sx * sy);
	}
	...
	Quat Quat::sLoadFloat3Unsafe(const Float3& inV)
	{
		...
		float w = std::sqrt(
		    std::max(1.0f - v.LengthSq(), 0.0f)); // It is possible that the length of v is a fraction above 1, and we
		                                          // don't want to introduce NaN's in that case so we clamp to 0
