...
namespace topia
{
	...
	namespace HalfFloatConversion
	{
		...
		inline HalfFloat FromFloatFallback(float inV)
		{
			...
			// Extract exponent
			u32 exponent = (value >> FLOAT_EXPONENT_POS) & FLOAT_EXPONENT_MASK;

			// Extract mantissa
			u32 mantissa = value & FLOAT_MANTISSA_MASK;
		...
		template <int RoundingMode>
		TOPIA_INLINE HalfFloat FromFloat(float inV)
		{
#ifdef TOPIA_USE_F16C
			union
			{
				__m128i u128;
				HalfFloat u16[8];
		...
		TOPIA_INLINE Vec4 ToFloat(UVec4Arg inValue)
		{
#if defined(TOPIA_USE_F16C)
